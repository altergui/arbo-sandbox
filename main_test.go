...
func TestMockProof(t *testing.T) {
	...
	k := arbo.BigIntToBytesLE(bLen, big.NewInt(int64(200)))
	...
	if string(jCvp) != `{"fnc":0,"isOld0":"0","key":"2","oldKey":"0","oldValue":`+
		...
		`"100272205498883598074187859649410185367157757359872275599596420039979979515420"],`+
		`"value":"22"}` {
		panic("")
	}
	...
	if string(jCvp) != `{"fnc":1,"isOld0":"0","key":"5","oldKey":"1",`+
		...
		`"61093584903451702856702332824323051875980263396096817106018703936895272494632"],`+
		`"value":"11"}` {
		panic("")
	}
}
